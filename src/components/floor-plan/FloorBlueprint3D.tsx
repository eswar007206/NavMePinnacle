...
function PersonPin({ position, label }: { position: [number, number, number]; label: string }) {
  ...
  useFrame(({ clock }) => {
    if (ref.current) ref.current.position.y = position[1] + Math.sin(clock.getElapsedTime() * 2 + off) * 0.08;
  });
  ...
  return (
    <group>
      ...
      <Text position={[position[0], position[1] + 0.35, position[2]]} fontSize={0.2} color="white" anchorX="center" anchorY="middle" outlineWidth={0.02} outlineColor="#1e3a5f" font={undefined}>
        {label}
      </Text>
    </group>
  );
