...
export default function EmergencySOS() {
  ...
  const activateEmergency = useCallback(async () => {
    ...
    // 1. Set emergency_state to active
    await supabase
      .from("emergency_state")
  ...
  const deactivateEmergency = useCallback(async () => {
    ...
    // 1. Set emergency_state to inactive
    await supabase
      .from("emergency_state")
      .update({ is_active: false, activated_at: null, updated_at: new Date().toISOString() })
      .eq("id", EMERGENCY_ROW_ID);
